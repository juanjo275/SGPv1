...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.documentoComercial.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.documento-comercials.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="monto">{{ trans('cruds.documentoComercial.fields.monto') }}</label>
                <input class="form-control {{ $errors->has('monto') ? 'is-invalid' : '' }}" type="number" name="monto" id="monto" value="{{ old('monto', '') }}" step="0.01">
                @if($errors->has('monto'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipo_documento_comercial') ? 'is-invalid' : '' }}" name="tipo_documento_comercial_id" id="tipo_documento_comercial_id">
                    @foreach($tipo_documento_comercials as $id => $tipo_documento_comercial)
                        <option value="{{ $id }}" {{ old('tipo_documento_comercial_id') == $id ? 'selected' : '' }}>{{ $tipo_documento_comercial }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estado_documento_comercials') ? 'is-invalid' : '' }}" name="estado_documento_comercials[]" id="estado_documento_comercials" multiple>
                    @foreach($estado_documento_comercials as $id => $estado_documento_comercial)
                        <option value="{{ $id }}" {{ in_array($id, old('estado_documento_comercials', [])) ? 'selected' : '' }}>{{ $estado_documento_comercial }}</option>
                    @endforeach
                </select>
