...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cuentaBancarium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cuenta-bancaria.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="cbu">{{ trans('cruds.cuentaBancarium.fields.cbu') }}</label>
                <input class="form-control {{ $errors->has('cbu') ? 'is-invalid' : '' }}" type="text" name="cbu" id="cbu" value="{{ old('cbu', '') }}">
                @if($errors->has('cbu'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nro_cuenta') ? 'is-invalid' : '' }}" type="text" name="nro_cuenta" id="nro_cuenta" value="{{ old('nro_cuenta', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('banco') ? 'is-invalid' : '' }}" name="banco_id" id="banco_id">
                    @foreach($bancos as $id => $banco)
                        <option value="{{ $id }}" {{ old('banco_id') == $id ? 'selected' : '' }}>{{ $banco }}</option>
                    @endforeach
                </select>
                @if($errors->has('banco'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipo_cuenta') ? 'is-invalid' : '' }}" name="tipo_cuenta_id" id="tipo_cuenta_id">
                    @foreach($tipo_cuentas as $id => $tipo_cuenta)
                        <option value="{{ $id }}" {{ old('tipo_cuenta_id') == $id ? 'selected' : '' }}>{{ $tipo_cuenta }}</option>
                    @endforeach
                </select>
