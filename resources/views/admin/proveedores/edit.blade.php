...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.proveedore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.proveedores.update", [$proveedore->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="cuit">{{ trans('cruds.proveedore.fields.cuit') }}</label>
                <input class="form-control {{ $errors->has('cuit') ? 'is-invalid' : '' }}" type="text" name="cuit" id="cuit" value="{{ old('cuit', $proveedore->cuit) }}">
                @if($errors->has('cuit'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('razon_social') ? 'is-invalid' : '' }}" type="text" name="razon_social" id="razon_social" value="{{ old('razon_social', $proveedore->razon_social) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefono') ? 'is-invalid' : '' }}" type="text" name="telefono" id="telefono" value="{{ old('telefono', $proveedore->telefono) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('domicilio') ? 'is-invalid' : '' }}" type="text" name="domicilio" id="domicilio" value="{{ old('domicilio', $proveedore->domicilio) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $proveedore->email) }}">
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cuenta_bancaria') ? 'is-invalid' : '' }}" name="cuenta_bancaria_id" id="cuenta_bancaria_id">
                    @foreach($cuenta_bancarias as $id => $cuenta_bancaria)
                        <option value="{{ $id }}" {{ (old('cuenta_bancaria_id') ? old('cuenta_bancaria_id') : $proveedore->cuenta_bancaria->id ?? '') == $id ? 'selected' : '' }}>{{ $cuenta_bancaria }}</option>
                    @endforeach
                </select>
